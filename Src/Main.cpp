...
int getClosestShape(float* min, Ray sampleRay, std::vector<Shape*>& shapes) {
...
Color directLightTransport(Light* LSource, int nbLights, int shpIndex, std::vector<Shape*>& shapes, Vector3 rayIntersec) {
...
Color diffuseLightTransport(int nbBounces, int shpIndex, std::vector<Shape*>& shapes, Vector3 rayIntersec) {
    ...
    for (int i = 0; i < nbSamples; i++)
    {
        ...
        newShapeIndex = getClosestShape(&min, sampleRay, shapes);
        Color found = Color::Black;
        ...
        else
        {
            ...
            //nbBounces++;
            Vector3 newIntersec = rayOrigin + randomDir * min;
            found = found * colorRetention * diffuseLightTransport(nbBounces + 1, newShapeIndex, shapes, newIntersec);
        }
...
Color mirrorLightTransport(Light* LSource, int nbLights, int shpIndex, std::vector<Shape*>& shapes, Vector3 rayIntersec, Vector3 rayDir) {
    ...
    int foundSphere = getClosestShape(&min, reflectedRay, shapes);
    //Color foundColor = Color::Black;
    Color foundColor = Color(255, 20, 147);
    ...
    if (foundSphere != -1)
    {
        ...
        foundColor = directLightTransport(LSource, nbLights, foundSphere, shapes, rayIntersec);
...
void readOBJ(std::string fileName, std::vector<Shape*>& myShapes) {
...
int main()
{
    ...
    Triangle Tri1(Vector3(200, 700, 1300), Vector3(200, 600, 1400), Vector3(400, 700, 1500), Color::Blue);

    //Shape* shapes[] = {/* &CenterSphere, &CenterDownSphere, */ &bSphere, &cSphere, &Tri1, &backGround ,&ceiling, &floor , &RWall,&LWall, &noPointLight };
    //int nbShapes = 8;

    std::vector<Shape*> shapes;
    shapes.push_back(&bSphere);
    shapes.push_back(&cSphere);
    ...
    for (int i = 0; i < h; i++) {
        ...
        for (int j = 0; j < w; j++) {
            ...
            for (int s = 0; s < nbSamples; s++)
            {
                ...
                float t = 99999999999;
                //shapeIndex = getClosestShape(&t, aRay, shapes, nbShapes);   // static array version
                shapeIndex = getClosestShape(&t, aRay, shapes);
                if (shapeIndex == -1) {
                ...
                {
                case DIFFUSE:
                    //sampleColor = diffuseLightTransport(nbBounces, shapeIndex, shapes, nbShapes, rayIntersec); // static array version
                    sampleColor = diffuseLightTransport(nbBounces, shapeIndex, shapes, rayIntersec);
                ...
                case MIRROR:
                    ...
                    //shapes[shapeIndex]->color = mirrorLightTransport(lights, nbLights, shapeIndex, shapes, nbShapes, rayIntersec, aRay.direction); // static array version
                    //sampleColor = directLightTransport(lights, nbLights, shapeIndex, shapes, nbShapes, rayIntersec); // static array version

                    shapes[shapeIndex]->color = mirrorLightTransport(lights, nbLights, shapeIndex, shapes, rayIntersec, aRay.direction); // static array version
                    sampleColor = directLightTransport(lights, nbLights, shapeIndex, shapes, rayIntersec); // static array version

                    break;

                case DIELECTRIC:

                    //sampleColor = dielectricsLightTransport(lights, nbLights, shapeIndex, shapes, nbShapes, rayIntersec, aRay.direction); // static array version
                    //sampleColor = dielectricsLightTransport(lights, nbLights, shapeIndex, shapes, rayIntersec, aRay.direction); // static array version
                    break;
